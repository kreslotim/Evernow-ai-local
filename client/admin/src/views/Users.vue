...
// Обработчик очистки данных пользователя
const handleClearUserData = async (userId: string) => {
  try {
    await ElMessageBox.confirm(
      'Это действие очистит все данные пользователя:\n\n• Результаты теста Люшера\n• Все анализы и гипотезы\n• Загруженные фотографии\n• Социальные карточки\n• История воронки\n\nПользователь останется в системе, но сможет начать процесс заново.\n\nВы уверены, что хотите продолжить?',
    ...
    const result = await usersStore.clearUserData(userId);
    ElMessage.success(`Данные пользователя успешно очищены. Удалено ${result.filesDeleted} файлов.`);

    // Обновляем список пользователей после очистки
    await fetchUsers();
  } catch (error) {
    if (error !== 'cancel' && error !== 'close') {
      console.error('Error clearing user data:', error);
      ElMessage.error('Не удалось очистить данные пользователя');
    }
  }
};

// Обработчик полного удаления пользователя (обнуление)
const handleDeleteUser = async (userId: string) => {
  try {
    await ElMessageBox.confirm(
      'ВНИМАНИЕ! Это действие полностью удалит пользователя и все связанные с ним данные из базы данных.\n\nПосле удаления пользователь сможет начать заново при вводе команды /start.\n\nВы уверены, что хотите продолжить?',
    ...
    ElMessage.success('Пользователь успешно удален из системы');
...
<template>
  <div>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
      ...
      <!-- Пустой результат -->
      <div v-else-if="users.length === 0" class="p-10 text-center text-gray-500">Пользователи не найдены</div>
      ...
      <div v-else>
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ID/Имя</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Статус</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Анализы</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Настройки
                </th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Рефералы</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Дата регистрации
                </th>
              ...
              <tr v-for="user in users" :key="user.id" :class="{ 'bg-red-50': user.isBanned }">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex flex-col">
                    ...
                    >
                      {{ user.isSubscribed ? 'Подписан на каналы' : 'Не подписан на каналы' }}
                    </span>
                    <span
                      v-if="user.isBanned"
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-gray-900">{{ user.analyzesCount || 0 }}</div>
                  <div class="text-sm text-gray-500">Выполнено анализов</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-gray-900">
                    <div>Кредитов: {{ user.analysisCredits }}</div>
                    <div class="text-sm text-gray-500">Роль: {{ user.role }}</div>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-gray-900">{{ user.referralsCount || 0 }}</div>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div class="flex gap-2 justify-end">
                    ...
                    <!-- Кнопка очистки данных пользователя -->
                    <button
                      @click="handleClearUserData(user.id)"
                      class="text-orange-600 hover:text-orange-800"
                      title="Очистить данные пользователя (тест Люшера, анализы, файлы)"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        class="h-5 w-5"
                    ...
                    <!-- Кнопка полного удаления пользователя (обнуление) -->
                    <button
                      @click="handleDeleteUser(user.id)"
                      class="text-red-800 hover:text-red-950"
                      title="ОБНУЛИТЬ ПОЛЬЗОВАТЕЛЯ - Полное удаление из БД"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        class="h-5 w-5"
        ...
        <div class="p-4 flex items-center justify-between border-t border-gray-200">
          <div class="flex-1 flex justify-between sm:hidden">
            ...
            <button
              @click="handlePageChange(filterOptions.page + 1)"
              :disabled="filterOptions.page * filterOptions.limit >= totalUsers"
              :class="[
                ...
                <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                  ...
                  <template v-for="pageNum in Math.ceil(totalUsers / filterOptions.limit)">
                    <button
                      v-if="
                        pageNum <= 3 ||
                        pageNum > Math.ceil(totalUsers / filterOptions.limit) - 3 ||
                    ...
                    <span
                      v-else-if="
                        (pageNum === 4 && filterOptions.page > 4) ||
                        (pageNum === Math.ceil(totalUsers / filterOptions.limit) - 3 &&
                          filterOptions.page < Math.ceil(totalUsers / filterOptions.limit) - 3)
                      "
